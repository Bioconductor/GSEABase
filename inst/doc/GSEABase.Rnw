...
\Rcode{hgu95av2} package. \Rfunction{mapIdentifiers} can automatically
determine many of the common maps; it is also possible to provide as a
third argument an environment containing an arbitrary map. Use the
...
@ 
%% 
This creates a gene set by consulting an environment in the
\Rpackage{GO} package. A gene set created from an expression set, and
...
contains gene symbols from band 24 of the q arm of chromsome 16. The
...
available in the \Rpackage{GSEABase} package.
