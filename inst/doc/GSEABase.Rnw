...
descriptive information about the set, and informatoin about how the
gene set was constructed. To construct a gene set, use
\Rfunction{GeneSet}. For example, to create a gene set from the all
identifiers in a subset of the sample \Robject{ExpressionSet} in the
Biobase package (see \Rpackage{Biobase} documentation for a
...
for carefully curating gene sets. Additonal informaton is
...
corresponding map in the annotation package exists), or to create a
gene set with particular identifiers:
<<mapIdentifiers>>=
mapIdentifiers(egs, EntrezIdentifier())
eids <- unique(unlist(mget(geneIds(egs), hgu95av2ENTREZID)))
eids <- eids[!is.na(eids)]
GeneSet(EntrezIdentifier(), geneIds=as.character(eids))
@ 
%% 
Use the \Rfunction{verbose} argument of \Rmethod{mapIdentifiers} to be informed when
the map between identifier types is not 1:1.

The \Rmethod{collectionType} of a gene set provides addiitonal
...
Broad Institute can be created as follows:
<<Broad>>=
...
@ 
%% 
The example above uses a local file, but it might also be a web
address accessible via the \Rcode{http://} protocol. The file name is
added to the url of the gene set. The set name and category of the
Broad collection indicate that the gene set contains gene symbols from
band 24 of the q arm of chromsome 16. The probe sets in chip
\Rcode{hgu95av2} corresponding to these symbols can be determined by
mapping identifiers
<<Broad-to-annotation>>=
...
Subsetting creates sets with just the sybmols identified. Subsetting
...
@ 
%% 
is empty, incidating that none of the identifiers in \Robject{egs} are
on \Rcode{16q24}. Additional operations on sets include union
(performed with \Rfunction{|}) and difference (\Rfunction{setdiff}).
...
subset is not surprising. Note also that subsetting requried mapping
...
consequences for phenotype refer to increasing or decreasing levels of
