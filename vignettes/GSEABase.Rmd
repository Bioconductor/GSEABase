...
package: GSEABase 1.59.0^[Report issues on https://github.com/Bioconductor/GSEABase/issues]
abstract: >
  The [_GSEABase_](http://bioconductor.org/packages/GSEABase) package implements data structures and methods to represent,
  manipulate, and analyze gene sets in the context of gene set
...
A _GeneSet_ stores a set of related gene identifiers. Important components of the
gene set are a vector of identifiers, general descriptive information
about the set, and information about how the gene set was constructed.
To construct a gene set, use `GeneSet` . For example, to create a gene set from
the identifiers in a subset of the sample _ExpressionSet_ in the [_Biobase_](http://bioconductor.org/packages/Biobase)
package use
...
Each gene set may have a name. The gene set contains identifiers ('genes')
from the _ExpressionSet_. These are accessible using `geneIds`, e.g.,

```{r geneIds}
head(geneIds(egs))
```

The gene set records that the identifiers are
probeset names from the annotation package [_hgu95av2.db_](http://bioconductor.org/packages/hgu95av2.db),
and that the source of the
gene set was an _ExpressionSet_. Additional details are available:
...
the [_/hgu95av2.db_](http://bioconductor.org/packages/hgu95av2.db) package.
...
_GOCollection_ class allows creation of gene sets based on gene ontology (GO) terms.
...
consulting an object in the [_GO.db_](http://bioconductor.org/packages/GO.db) package.
...
band 24 of the q arm of chromosome 16. The probe sets in chip
...
Remember that `sample.ExpressionSet`  contains just probe sets, so the small size
of the subset is not surprising. Note also that subsetting required mapping of symbol
identifiers in to s; this map used the annotation information in the
expression set, and is not necessarily 1:1.

# _GeneColorSet_

A _GeneColorSet_ is a gene set with "coloring" to indicate how features of genes and
...
expression sets (provided the map between identifiers is 1:1, so that
coloring corresponding to identifiers can be determined). The `coloring` method
...
A _GeneSetCollection_ is a collection of gene sets. Sets in the collection must have
distinct `setName`s, but can be a mix of _GeneSet_ and _GeneColorSet_. Two convenient
ways to create a gene set collection are by specifying a source of identifiers (e.g., an
_ExpressionSet_ or _AnnotationIdentifier_) and how the identifiers are to be induced into sets (e.g., by
consulting the GO or KEGG ontologies):
...
In this example, the annotation identifiers of the sample expression set are organized
into gene sets based on their presence in GO pathways. Providing
arguments such as `evidenceCode` to _GOCollection_ act to select just those pathways
satisfying the GO collection constraint:
...
A convenient way to visualize a _GeneSetCollection_ is with the
[_ReportingTools_](http://bioconductor.org/packages/ReportingTools) package.
...
available in the [_GSEABase_](http://bioconductor.org/packages/GSEABase) package. 
