...
the identifiers in a subset of the sample _ExpressionSet_ in the `r Biocpkg('Biobase')`
package use
...
Each gene set may have a name. The gene set contains `r length(geneIds(egs))` identifiers ('genes')
...
probeset names from the annotation package `r Biocpkg('hgu95av2.db')`,
...
A gene set created from an expression set, and with collection type _GOCollection_
...
available in the `r Biocpkg('_GSEABase_')` package.
